...
from sklearn.kernel_approximation import RBFSampler,Nystroem  #カーネル近似用の関数
...
if method=='rff':
    rbf_feature = RBFSampler(gamma=1/sigma, n_components=n_components, random_state=1)

elif method=='nys':
    rbf_feature = Nystroem(kernel='rbf', gamma=1/sigma, n_components=n_components, random_state=1)

    
temp2 = rbf_feature.fit_transform(X_std)
...
X_train, X_test, train_label, test_label=cross_validation.train_test_split(X_std,z, test_size=0.3, random_state=1)
